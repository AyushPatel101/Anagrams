...
public class AnagramMain {
	...
	/*
	 * pre: words != null, console connected to System.in. words contains
	 * dictionary. Words will be emptied by the time this method completes. main
	 * loop of program. Called after words are loaded from file. Asks user for
	 * phrase and max number of words until phrase with 0 characters entered.
	 */
	private static void solveAnagrams(Set<String> words, Scanner console) {
		if (words == null) {
			throw new NullPointerException(
					"The parameter words cannot be null");
		}
	...
	/*
	 * Display the resulting anagrams from a given phrase. pre: anagrams != null
	 */
	private static void showAnagrams(List<List<String>> anagrams, int max,
			String phrase) {
		if (anagrams == null) {
			throw new NullPointerException(
					"The parameter anagrams (List of List of Strings) "
							+ "cannot be null.");
		}
		System.out.print(
				"Found " + anagrams.size() + " anagram(s) " + "for " + phrase);
		if (max != 0) {
			System.out.println(
					" with a " + "limit of no more than " + max + " words.");
		} else {
	...
	/*
	 * pre: dictionaryFileName != null Ask for a file name and read words from
	 * that file. returns a HashSet of the String from the file.
	 */
	public static HashSet<String> readWords(String dictionaryFileName) {
		if (dictionaryFileName == null) {
			throw new NullPointerException(
					"The parameter dictionaryFileName (String) "
							+ "cannot be null.");
		}
